...
foreach ($other in @('RetrySmoke.dll','ConvenienceSmoke.dll','DiceSmoke.dll','OtherSmoke.dll')) {
    ...
    if ($other -ne 'ConvenienceSmoke.dll' -and (Test-Path -LiteralPath $old)) { Move-Item -LiteralPath $old -Destination (Join-Path $disabled $other) -Force }
}
...
$rsp += '/out:"' + (Join-Path $PSScriptRoot 'dist/ConvenienceSmoke.dll') + '"'
$rsp += '"' + (Join-Path $PSScriptRoot 'ConvenienceSmoke.cs') + '"'
...
Copy-Item (Join-Path $PSScriptRoot 'dist/ConvenienceSmoke.dll') (Join-Path $runtime 'BepInEx/plugins')
