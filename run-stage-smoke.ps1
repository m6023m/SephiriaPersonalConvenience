param([switch]$UiOnly, [string]$Prepared = 'CombatTestArena/prepared-ddfeb3926fd3483e9c5d0d61884b2f5a')
...
foreach ($other in @('RetrySmoke.dll','StageSmoke.dll','ConvenienceSmoke.dll','DiceSmoke.dll','OtherSmoke.dll')) {
    $old = Join-Path $runtime ('BepInEx/plugins/' + $other)
    if ($other -ne 'StageSmoke.dll' -and (Test-Path -LiteralPath $old)) { Move-Item -LiteralPath $old -Destination (Join-Path $disabled $other) -Force }
}
...
$rsp += '/out:"' + (Join-Path $PSScriptRoot 'dist/StageSmoke.dll') + '"'
$rsp += '"' + (Join-Path $PSScriptRoot 'StageSmoke.cs') + '"'
...
Copy-Item (Join-Path $PSScriptRoot 'dist/StageSmoke.dll') (Join-Path $runtime 'BepInEx/plugins')
$start = [Diagnostics.ProcessStartInfo]::new()
$start.FileName = Join-Path $runtime 'Sephiria.exe'
$start.WorkingDirectory = $runtime
$start.UseShellExecute = $false
$start.WindowStyle = [Diagnostics.ProcessWindowStyle]::Hidden
foreach ($arg in @('-batchmode','-screen-fullscreen','0','-screen-width','1280','-screen-height','800','-logFile',(Join-Path $root 'stage-unity.log'))) { $start.ArgumentList.Add($arg) }
...
@{ProcessId=$process.Id;Root=$root} | ConvertTo-Json | Set-Content (Join-Path $root 'stage-process.json')
