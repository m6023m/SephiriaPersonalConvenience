...
foreach ($other in @('RetrySmoke.dll','ConvenienceSmoke.dll','DiceSmoke.dll','OtherSmoke.dll')) {
    ...
    if ($other -ne 'OtherSmoke.dll' -and (Test-Path -LiteralPath $old)) { Move-Item -LiteralPath $old -Destination (Join-Path $disabled $other) -Force }
}
...
$rsp += '/out:"' + (Join-Path $PSScriptRoot 'dist/OtherSmoke.dll') + '"'
$rsp += '"' + (Join-Path $PSScriptRoot 'OtherSmoke.cs') + '"'
...
Copy-Item (Join-Path $PSScriptRoot 'dist/OtherSmoke.dll') (Join-Path $runtime 'BepInEx/plugins')
$start = [Diagnostics.ProcessStartInfo]::new()
$start.FileName = Join-Path $runtime 'Sephiria.exe'
$start.WorkingDirectory = $runtime
$start.UseShellExecute = $false
$start.WindowStyle = [Diagnostics.ProcessWindowStyle]::Hidden
foreach ($arg in @('-batchmode','-screen-fullscreen','0','-screen-width','1280','-screen-height','800','-logFile',(Join-Path $root 'other-dice-unity.log'))) { $start.ArgumentList.Add($arg) }
