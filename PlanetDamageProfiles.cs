...
namespace SephiriaDicePreview
{
    internal static class PlanetDamageProfiles
    {
        internal static bool TryDescribe(Charm_SummonGreenBat source,Charm_SummonGreenBat live,ItemEntity entity,int level,PlayerAvatar p,out string text)
        {
            ...
            b.AppendLine(ProjectileDamageProfiles.Describe(p,Hit(raw,ignite,ticks,root,planet,false,bat.elementalType),Prefab(bat,false,ticks>0),"일반 상태"));
            b.AppendLine(ProjectileDamageProfiles.Describe(p,Hit(raw,ignite,ticks,root,planet,true,bat.elementalType),Prefab(bat,true,ticks>0),"강화 상태"));
        ...
        private static UnityEngine.GameObject Prefab(GreenBat bat,bool enhanced,bool ignite)
        {
